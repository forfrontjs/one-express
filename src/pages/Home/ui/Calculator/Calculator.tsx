...
export const Calculator: React.FC = () => {
  ...
  return (
    <div className={styles.cargoCalculator}>
      ...
      <div className={styles.calculator_skeleton}>
        <div className={styles.input_groupCalculator}>
          <div className={styles.button_CalculatorMain}>
            ...
            >
              <span className={styles.text_Calculator}>Объем плотности (p)</span>
            </button>
          </div>
            ...
            {!isDensityMode ? (
              <>
                ...
                <div className={styles.inputGroup}>
                  <input
                    type="number"
                    placeholder={placeholdersVisible.width ? "Ширина (м)" : ""}
                    className={styles.width_calculator}
                    value={width || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, width: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, width: true }))}
                ...
                <div className={styles.inputGroup}>
                  <input
                    type="number"
                    placeholder={placeholdersVisible.length ? "Длина (м)" : ""}
                    className={styles.length_calculator}
                    value={length || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, length: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, length: true }))}
                ...
                <div className={styles.inputGroup}>
                  <input
                    type="number"
                    placeholder={placeholdersVisible.height ? "Высота (м)" : ""}
                    className={styles.height_calculator}
                    value={height || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, height: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, height: true }))}
                ...
                <div className={styles.inputGroup}>
                  <input
                    type="number"
                    placeholder={placeholdersVisible.quantity ? "Количество" : ""}
                    className={styles.quantity_calculator}
                    value={quantity || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, quantity: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, quantity: true }))}
                    onChange={(e) => handleInputChange(e, setQuantity, "quantity")}
                  />
                </div>
                <div className={styles.inputGroup}>
                  <input
                    ...
                    value={price || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, price: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, price: true }))}
              ...
              <>
                ...
                <div className={styles.inputGroup}>
                  <input
                    type="number"
                    placeholder={placeholdersVisible.densityWeight ? "Вес груза (кг)" : ""}
                    className={styles.weight_calculatorSub}
                    value={densityWeight || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, densityWeight: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, densityWeight: true }))}
                    onChange={(e) => handleInputChange(e, setDensityWeight, "densityWeight")}
                  />
                </div>
                <div className={styles.inputGroup}>
                  <input
                    type="number"
                    placeholder={placeholdersVisible.densityVolume ? "Объем груза (м³)" : ""}
                    className={styles.volume_calculator}
                    value={densityVolume || ""}
                    onFocus={() => setPlaceholdersVisible((prev) => ({ ...prev, densityVolume: false }))}
                    onBlur={() => setPlaceholdersVisible((prev) => ({ ...prev, densityVolume: true }))}
                    onChange={(e) => handleInputChange(e, setDensityVolume, "densityVolume")}
                  />
                </div>
                <div className={styles.block_subgroupDensity}>
                  <span className={styles.block_subgroupTextDensity}>
                    {densityVolume ? `${densityResult.toFixed(2)} кг/м³` : "0 кг/м³"}
                  </span>
                </div>
              </>
            )}
