...
const Profile: React.FC = () => {
  ...
  const [statistics, setStatistics] = useState({ orders: 0, weight: 0, paid: 0 });
  ...
  const handleCopyClick = () => {
    navigator.clipboard.writeText('ONE-1').then(() => {
  ...
  const fetchStatistics = async (month: string) => {
    try {
      const response = await fetch(`http://192.168.68.236:5009/statistics?month=${month}`);
      if (!response.ok) {
  ...
  <path d="M8.94305 10.2762C9.57803 9.64122 10.4313 9.32903 11.3272 9.33335C12.2212 9.33766 13.087 9.65726 13.737 10.2893L14.693 11.2188L16.552 9.30694L15.5961 8.37743C14.4054 7.21966 12.8568 6.67401 11.3401 6.66671C9.81521 6.65936 8.2513 7.19667 7.05738 8.39055L1.72391 13.7238C0.51974 14.928 0 16.4905 0 18C0 19.5095 0.519753 21.072 1.72393 22.2761C2.92811 23.4803 4.49066 24 6.00016 24C7.50968 24 9.07225 23.4803 10.2764 22.2761L11.2193 21.3333L9.3336 19.4477L8.39077 20.3905C7.75395 21.0273 6.90358 21.3333 6.00017 21.3333C5.09676 21.3333 4.24641 21.0273 3.6096 20.3905C2.97277 19.7537 2.66674 18.9034 2.66674 18C2.66674 17.0966 2.97276 16.2463 3.60958 15.6095L8.94305 10.2762Z" fill="#F5D100" />
      ...
      <div className={styles.Statistics}>
        <div className={styles.StatisticsButton}>
          <h2 className={styles.YourStatistics}>Ваша статистика за {selectedMonth}</h2>
          <button onClick={toggleDropdown} className={styles.changeMonth}>
            ...
            <span>
              <svg width="18" height="18" viewBox="0 0 23 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13.178 19.1769L13.178 0.722049H10.2781L10.2781 19.1769L2.50058 11.3994L0.45003 13.4499L11.728 24.7279L23.006 13.4499L20.9555 11.3994L13.178 19.1769Z" fill="#626262" />
              </svg>
            </span>
          </button>
          ...
          {isDropdownOpen && (
            <ul className={styles.dropdownMenu}>
              {months.map((month) => (
                <li key={month} onClick={() => handleMonthSelect(month)}>{month}</li>
              ))}
            </ul>
          )}
        </div>
